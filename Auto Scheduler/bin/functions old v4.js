...
class Resident {
    ...
    hasGoldenWeekend() {
        ...
        for (let week = 0; week < 4; week++) {
            day = month[week * 7];
            if (!this.isWorkingThisWeekend(day)) return true;
        }
...
function createDropdown(resident, argStatus = 'full') {
...
function populateMonth() {
    ...
    for (let week = 0; week < 4; week++) {
        ...
        for (let i = (week * 7) + 4; i < (week * 7) + 6; i++) {
            ...
            //choose the 24hr sr/jr
            sr24 = choose24Sr(residents, day);
            jr24 = choose24Jr(residents, day);

            //if Fri, assign to respective team staff shift
            if (day.dayNum() == 4) {
                //senior (unless weekday chief already working)
                if (!empty(sr24)) {
                    if (sr24 != day.blueChief[0] && sr24 != day.redChief[0]) {
                        day[sr24.team + 'Staff'].push(assignShift(sr24, day));
                    }
                }
                //junior
                if (!empty(jr24)) day[jr24.team + 'Staff'].push(assignShift(jr24, day));
            }
            //if Sat, assign to respective team staff weekend shift, if same team put sr on consults
            if (day.dayNum() == 5) {

                //junior
                if (!empty(jr24)) day[jr24.team + 'Staff'].push(assignWeekend(jr24, day));

                //senior
                if (!empty(sr24)) {
                    if (day[sr24.team + 'Staff'].length > 0) day.consult.push(assignConsult(sr24, day));
                    else day[sr24.team + 'Staff'].push(assignWeekend(sr24, day));
                }
            }


            //assign to night shift
            if (!empty(sr24)) day.nightSr.push(assignNight(sr24, day));
            if (!empty(jr24)) day.nightJr.push(assignNight(jr24, day));
        }

        //4. assign weekend staff (blue, red, consults)
        for (let i = (week * 7) + 5; i < (week * 7) + 7; i++) {
            day = month[i];
            //blue Staff
        ...
        for (let i = week * 7; i < (week * 7) + 5; i++) {
            ...
            //assign junior (unless there are 2 seniors on staff by chance)
            if (!day.has('blue', 'jr') && day.blueStaff.length < 2) {
                chosen = chooseJr(blueTeam, day);
                if (!empty(chosen)) day.blueStaff.push(assignShift(chosen, day));
            }
            if (!day.has('red', 'jr') && day.redStaff.length < 2) {
...
function assignConsult(resident, day) {
    ...
    if (day.isSatSun()) return assignWeekend(resident, day);

    else return assignShift(resident, day);
}
...
function sortByNights(residents) {
    ...
    var output = [];
    for (let resident of residents) {
        output.push(String(resident._nights()) + '-' + resident.name);
    }
    console.log(output);
...
function choose24Sr(argTeam, day) {
    ...
    for (let resident of argTeam) {
        if (resident.canNightSr() && resident.isAvailableNight(day)) {
            if (day.dayNum() != 5 || resident.PGY != 5 || resident.team == 'edu') {
                return resident;
            }
        }
    }
    ...
    for (let position of positions) {
        for (let i = 0; i < month[day][position].length; i++) {
            ...
            if (!empty(resident)) {
                ...
                nameHTML.addEventListener('click', () => removeResident(resident, month[day]));
...
function makeFloatPoolRow(argWeek) {
    ...
    for (let day = (argWeek * 7); day < (argWeek * 7) + 7; day++) {
        ...
        for (let resident of residents) {
            if (resident.isAvailableDay(month[day])) {
        ...
        for (let team of teams) {
            for (let resident of floatPool[team]) {
                ...
                //custom right click menu - default no display
                container = document.getElementById('custom-menus');
                menu = document.createElement('div');
                menu.setAttribute('id', 'custom-menu-' + resident.name);
                ...
                for (let position of positions) {
                    ...
                    option.addEventListener('click', function () {
                        addResident(resident, month[day], position);
                    });
                ...
                residentHTML.addEventListener('contextmenu', function (event) {
                    ...
                    customMenu = document.getElementById('custom-menu-' + resident.name);
                ...
                document.addEventListener('click', function () {
                    document.getElementById('custom-menu-' + resident.name).style.display = 'none';
                });
...
function choosePosition(resident, day) {
    ...
    //choose what position to place resident
    //var teamColor = resident.teamColor;
...
function removeResident(resident, day) {
    ...
    day[residentPosition].splice(residentIndex, 1);

    //remove shifts from resident Object
    unassign(resident, day, residentPosition);
...
function importData() {
    ...
    //trigger hidden file input element
...
function handleFileSelection(files) {
    ...
    reader.onload = function (e) {
        ...
        //To Do: clear availability, teams, schedule, stats
        residents = [];        
        document.getElementById('all-residents').innerHTML = '';
        clearSchedule();
        ...
        //convert each resident json to a pointer to the resident Object
        month = [];
