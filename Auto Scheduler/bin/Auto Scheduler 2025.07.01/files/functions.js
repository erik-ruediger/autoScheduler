...
/* global data */ {
    ...
    if (redFirst) {
        //positions = ['redChief', 'redStaff', 'blueChief', 'blueStaff', 'consults', 'nightSr', 'nightJr'];
        positions = ['redChief', 'blueChief', 'consults', 'redStaff', 'blueStaff', 'nightSr', 'nightJr'];
    }
    else positions = ['blueChief', 'blueStaff', 'redChief', 'redStaff', 'consults', 'nightSr', 'nightJr'];
...
function createDropdown(resident) {
    ...
    if (redFirst) values = ['---', 'red', 'blue', 'edu'];
    else values = ['---', 'blue', 'red', 'edu'];
    ...
    for (let week = 0; week < 4; week++) {
        ...
        for (let i = (week * 7) + 5; i > (week * 7) + 3; i--) {
            ...
            if (day.dayNum() == 5) {
                ...
                else day.consults.push(assignConsult(sr24, day));
            }
        }
        ...
        for (let i = (week * 7) + 5; i < (week * 7) + 7; i++) {
            ...
            if (day.consults.length == 0) {
                ...
                if (!empty(chosen)) day.consults.push(assignConsult(chosen, day));
            }
        }
...
function assignShift(resident, day) {
    ...
    //if tuesday, record tuesday shift
...
function assignConsult(resident, day) {
    ...
    //recursively record shift (this will catch tuesdays)
    return assignShift(resident, day);
}
function unassign(resident, day, argPosition) {
    ...
    if (argPosition.includes('consults')) resident.consults[week] -= 1;
    if (day.isSatSun()) resident.weekends[week] -= 1;
    if (day.isTue()) resident.tuesdays[week] -= 1;
}
...
function makeFloatPoolRow(argWeek) {
    ...
    if (redFirst) teams = ['red', 'blue', 'edu', '---'];
    else['blue', 'red', 'edu', '---'];
...
function addResident(resident, day, residentPosition = '') {
    ...
    else if (residentPosition.includes('consults')) day[residentPosition].push(assignConsult(resident, day));
    //assign weekend
